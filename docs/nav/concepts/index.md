...
- **[Config](concept-config.md)** — the dependency-injection container that wires a provider and a tool set together, the same pattern dotflow uses for `Storage`/`Notify`/`Log`.
