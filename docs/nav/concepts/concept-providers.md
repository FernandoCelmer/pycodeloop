...
|----------|-------|-------|
| Anthropic | `pycodeloop.providers.AnthropicProvider` | Claude models, native streaming and tool use |
| OpenAI | `pycodeloop.providers.OpenAIProvider` | GPT models; `base_url` makes it work with any OpenAI-compatible server |
| Ollama | `pycodeloop.providers.OllamaProvider` | `OpenAIProvider` pointed at a local Ollama server by default |
| Generic | `pycodeloop.providers.GenericProvider` | Any HTTP endpoint; pluggable request/response shape, real SSE streaming |
