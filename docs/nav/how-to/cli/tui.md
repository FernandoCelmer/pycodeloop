...
Start the full-screen [Textual](https://textual.textualize.io/) interface. This is what bare `codeloop` runs — `tui` is the explicit form when you want to pass options without triggering a subcommand parse ambiguity.
...
Same options as [`codeloop run`](run.md):

```bash
codeloop tui --provider openai --model gpt-5
codeloop tui --mcp "npx -y @modelcontextprotocol/server-filesystem ." --yes
```
