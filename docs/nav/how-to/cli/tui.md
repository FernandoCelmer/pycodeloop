...
Start the full-screen [Textual](https://textual.textualize.io/) interface. This is what bare `pycodeloop` runs — `tui` is the explicit form when you want to pass options without triggering a subcommand parse ambiguity.
...
```bash
pycodeloop tui --provider openai --model gpt-5
pycodeloop tui --mcp "npx -y @modelcontextprotocol/server-filesystem ." --yes
```
